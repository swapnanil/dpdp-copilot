...
export function renderEvidenceHtml({ org, request, evidence }) {
  ...
  const requestStatus = escapeHtml(request.sla_status)
  const message = escapeHtml(request.message)
  const suggestedReply = escapeHtml(request.suggested_reply)
...
<body>
  ...
  <div class="meta">
    ...
    <div><strong>Type:</strong> ${requestType}</div>
  ...
  ${
    ...
    <div class="section">
      ...
      <div class="box">${suggestedReply}</div>
    </div>
  `
      : ''
  }
